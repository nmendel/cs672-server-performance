...
def run_benchmark(benchmark, test_config=TestConfig(), context=None, *args, **kwargs):
    ...
    #stop = now.replace(hour=now.hour+1)
    stop = now.replace(minute=now.minute+1)

    logger.info('Benchmark: ' + message + ' starting at: ' + now.strftime("%Y-%m-%d %H:%M:%S") + ', stopping at: ' + stop.strftime("%Y-%m-%d %H:%M:%S"))
    print 'Benchmark: ' + message + ' starting at: ' + now.strftime("%Y-%m-%d %H:%M:%S") + ', stopping at: ' + stop.strftime("%Y-%m-%d %H:%M:%S")
    ...
    return analyze_benchmark_results(output, benchmark, total_requests, now.strftime("%Y-%m-%d %H:%M:%S"), stop.strftime("%Y-%m-%d %H:%M:%S"))


def analyze_benchmark_results(benchmark_result, benchmark, total_requests, start, stop):
    ...
    aggregate_results.append(('test-time', 'start', start))
    aggregate_results.append(('test-time', 'stop', stop))
    aggregate_results.append(('requests', 'sum', total_requests))
